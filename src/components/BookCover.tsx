...
export function BookCover({ book, width = "w-16", height = "h-24", className = "" }: BookCoverProps) {
    ...
    return (
      <img
        ...
        onError={(e) => {
          ...
          if (parent) {
            parent.innerHTML = createDynamicCover(book);
          }
        }}
      />
    );
  ...
  const createDynamicCover = (book: Book) => {
    ...
    return `
      <div class="${width} ${height} ${pattern} ${gradient} rounded-lg shadow-md flex flex-col justify-between p-2 text-white relative overflow-hidden ${className}">
        ${hasPattern ? `
        ...
        ` : ''}
        <div class="text-center flex-1 flex items-center justify-center relative z-10">
          <div class="text-2xl font-bold opacity-90 bg-white bg-opacity-20 w-8 h-8 rounded-full flex items-center justify-center text-sm">
            ${firstChar}
          </div>
        </div>
        <div class="text-[10px] font-medium text-center leading-tight opacity-90 relative z-10">
          ${shortTitle}
        </div>
        <div class="text-[8px] text-center opacity-75 truncate relative z-10">
          ${book.author}
        </div>
      </div>
    `;
  ...
  return (
    <div
      dangerouslySetInnerHTML={{
        __html: createDynamicCover(book)
      }}
    />
  );
}
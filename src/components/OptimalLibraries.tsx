          ...
          <CardHeader className="pb-3 sm:pb-6">
            <CardTitle className="flex items-center justify-between text-base sm:text-lg">
              ...
              <Badge className="bg-gradient-to-r from-green-500 to-emerald-500 text-white text-xs sm:text-sm">
                커버리지 {set.coverageRate}%
              </Badge>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-4 sm:space-y-6 pt-0">
            {set.books.map((book) => (
              <div key={book.isbn} className="space-y-3 sm:space-y-4">
                <div className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 bg-white rounded-lg shadow-sm">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="font-semibold text-gray-900 mb-1 sm:mb-2 text-sm sm:text-base line-clamp-2">
                      {book.title}
                    </h4>
                    <p className="text-xs sm:text-sm text-gray-600">
                      {book.libraries.length}개 도서관에서 대출 가능
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-2 sm:space-y-3 ml-2 sm:ml-4">
                  {book.libraries.map((library) => (
                    <div
                      key={library.lib_code}
                      className="flex items-start gap-2 sm:gap-3 p-3 bg-blue-50 rounded-lg border border-blue-100"
                    >
                      <Building2 className="w-4 h-4 sm:w-5 sm:h-5 text-blue-600 mt-0.5 flex-shrink-0" />
                      <div className="flex-1 min-w-0">
                        <h5 className="font-medium text-blue-900 mb-1 text-sm sm:text-base line-clamp-2">
                          {library.lib_name}
                        </h5>
